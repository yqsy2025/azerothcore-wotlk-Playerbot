...
int Mute(lua_State* L, Player* player)
{
    ...
    LoginDatabase.Execute("UPDATE account SET mutetime = {} WHERE id = {}", muteTime,
...
/**
 * Adds a glyph specified by `glyphId` to the [Player]'s current talent specialization into the slot with the index
 * `slotIndex`
 *
...
/**
 * Remove cooldowns on spells that have less than 10 minutes of cooldown from the [Player], similarly to when you enter
 * an arena.
 */
 ...
 *
 * sender and intid are numbers which are passed directly to the gossip selection handler. Internally they are partly
 * used for the database gossip handling. code specifies whether to show a box to insert text to. The player inserted
 * text is passed to the gossip selection handler. money specifies an amount of money the player needs to have to click
 * the option. An error message is shown if the player doesn't have enough money. Note that the money amount is only
 * checked client side and is not removed from the player either. You will need to check again in your code before
 * taking action.
 *
...
int GossipMenuAddItem(lua_State* L, Player* player)
{
    ...
    {
        return luaL_error(
            L, "GossipMenuItem not added. Reached Max amount of possible GossipMenuItems in this GossipMenu");
    }
    return 0;
}
 ...
 *
 *     Note: This is needed when you show a gossip menu without using gossip hello or select hooks which do this
 * automatically. Usually this is needed when using [Player] is the sender of a Gossip Menu.
 */
...
int TeleportTo(lua_State* L, Player* player)
{
    ...
    player->TeleportTo(game_tele->mapId, game_tele->position_x, game_tele->position_y, game_tele->position_z,
                       game_tele->orientation);
    return 0;
}
...
int CalculateReputationGain(lua_State* L, Player* player)
{
    ...
    ALE::Push(
        L, player->CalculateReputationGain((ReputationSource)source, creatureOrQuestLevel, rep, faction, noQuestBonus));
    return 1;
}
