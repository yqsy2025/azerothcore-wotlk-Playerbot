...
enum PacketEvents
{
    PACKET_EVENT_ON_PACKET_RECEIVE =
        5,  // (event, packet, player) - Player only if accessible. Can return false, newPacket
    PACKET_EVENT_ON_PACKET_RECEIVE_UNKNOWN = 6,  // Not Implemented
    PACKET_EVENT_ON_PACKET_SEND =
        7,  // (event, packet, player) - Player only if accessible. Can return false, newPacket
...
enum ServerEvents
{
    ...
    AUCTION_EVENT_ON_ADD =
        26,  // (event, auctionId, owner, item, expireTime, buyout, startBid, currentBid, bidderGUIDLow)
    AUCTION_EVENT_ON_REMOVE =
        27,  // (event, auctionId, owner, item, expireTime, buyout, startBid, currentBid, bidderGUIDLow)
    AUCTION_EVENT_ON_SUCCESSFUL =
        28,  // (event, auctionId, owner, item, expireTime, buyout, startBid, currentBid, bidderGUIDLow)
    AUCTION_EVENT_ON_EXPIRE =
        29,  // (event, auctionId, owner, item, expireTime, buyout, startBid, currentBid, bidderGUIDLow)

    // AddOns
    ADDON_EVENT_ON_MESSAGE = 30,  // (event, sender, type, prefix, msg, target) - target can be
...
enum PlayerEvents
{
    ...
    PLAYER_EVENT_ON_REPUTATION_CHANGE = 15,  // (event, player, factionId, standing, incremental) - Can return new
                                             // standing -> if standing == -1, it will prevent default action (rep gain)
    ...
    PLAYER_EVENT_ON_CHANNEL_CHAT = 22,  // (event, player, msg, Type, lang, channel) - channel is negative for custom
                                        // channels. Can return false, newMessage
    ...
    PLAYER_EVENT_ON_COMMAND =
        42,  // (event, player, command, chatHandler) - player is nil if command used from console. Can return false
    ...
    PLAYER_EVENT_ON_CAN_SEND_MAIL = 49,  // (event, player, receiverGuid, mailbox, subject, body, money, cod, item) -
                                         // Can return false to prevent sending the mail
    PLAYER_EVENT_ON_CAN_JOIN_LFG =
        50,  // (event, player, roles, dungeons, comment) - Can return false to prevent queueing
    ...
    PLAYER_EVENT_ON_MODIFY_PERIODIC_DAMAGE_AURAS_TICK =
        68,  // (event, player, target, damage, spellInfo) - Can return new damage amount
    ...
    PLAYER_EVENT_ON_MODIFY_SPELL_DAMAGE_TAKEN =
        70,  // (event, player, target, damage, spellInfo) - Can return new damage amount
...
enum GuildEvents
{
    ...
    GUILD_EVENT_ON_ITEM_MOVE = 9,       // (event, guild, player, item, isSrcBank, srcContainer, srcSlotId, isDestBank,
                                        // destContainer, destSlotId)   // TODO
    GUILD_EVENT_ON_EVENT = 10,          // (event, guild, eventType, plrGUIDLow1, plrGUIDLow2, newRank)  // TODO
    GUILD_EVENT_ON_BANK_EVENT =
        11,  // (event, guild, eventType, tabId, playerGUIDLow, itemOrMoney, itemStackCount, destTabId)
...
enum CreatureEvents
{
    ...
    CREATURE_EVENT_ON_DAMAGE_TAKEN = 9,  // (event, creature, attacker, damage) - Can return true to stop normal action,
                                         // can return new damage as second return value.
    ...
    CREATURE_EVENT_ON_OWNER_ATTACKED =
        12,  // (event, creature, target) - Can return true to stop normal action            // Not on mangos
    CREATURE_EVENT_ON_OWNER_ATTACKED_AT =
        13,  // (event, creature, attacker) - Can return true to stop normal action          // Not on mangos
    ...
    CREATURE_EVENT_ON_SPELL_HIT_TARGET =
        15,  // (event, creature, target, spellid) - Can return true to stop normal action
    ...
    CREATURE_EVENT_ON_SUMMONED_CREATURE_DESPAWN =
        20,  // (event, creature, summon) - Can return true to stop normal action
    CREATURE_EVENT_ON_SUMMONED_CREATURE_DIED =
        21,  // (event, creature, summon, killer) - Can return true to stop normal action    // Not on mangos
    ...
    CREATURE_EVENT_ON_CORPSE_REMOVED = 26,  // (event, creature, respawndelay) - Can return true to stop normal action,
                                            // can return new respawndelay as second return value
    CREATURE_EVENT_ON_MOVE_IN_LOS = 27,     // (event, creature, unit) - Can return true to stop normal action. Does not
                                            // actually check LOS, just uses the sight range
    ...
    CREATURE_EVENT_ON_MODIFY_PERIODIC_DAMAGE_AURAS_TICK =
        42,  // (event, creature, target, damage, spellInfo) - Can return new damage amount
    CREATURE_EVENT_ON_MODIFY_MELEE_DAMAGE = 43,  // (event, creature, target, damage) - Can return new damage amount
    CREATURE_EVENT_ON_MODIFY_SPELL_DAMAGE_TAKEN =
        44,  // (event, creature, target, damage, spellInfo) - Can return new damage amount
    CREATURE_EVENT_ON_MODIFY_HEAL_RECEIVED =
        45,                              // (event, creature, target, heal, spellInfo) - Can return new heal amount
...
enum GameObjectEvents
{
    ...
    GAMEOBJECT_EVENT_ON_DUMMY_EFFECT =
        3,  // (event, caster, spellid, effindex, go) - Can return true to stop normal action
...
enum GossipEvents
{
    GOSSIP_EVENT_ON_HELLO = 1,  // (event, player, object) - Object is the Creature/GameObject/Item. Can return false to
                                // do default action. For item gossip can return false to stop spell casting.
    GOSSIP_EVENT_ON_SELECT =
        2,  // (event, player, object, sender, intid, code, menu_id) - Object is the Creature/GameObject/Item/Player,
            // menu_id is only for player gossip. Can return false to do default action.
    GOSSIP_EVENT_COUNT
};
...
enum AllCreatureEvents
{
    ...
    ALL_CREATURE_EVENT_ON_BEFORE_SELECT_LEVEL =
        4,                                  // (event, creature_template, creature, level) - Can return the new level
    ...
    ALL_CREATURE_EVENT_ON_MODIFY_PERIODIC_DAMAGE_AURAS_TICK =
        9,  // (event, creature, target, damage, spellInfo) - Can return new damage amount
    ALL_CREATURE_EVENT_ON_MODIFY_MELEE_DAMAGE = 10,  // (event, creature, target, damage) - Can return new damage amount
    ALL_CREATURE_EVENT_ON_MODIFY_SPELL_DAMAGE_TAKEN =
        11,  // (event, creature, target, damage, spellInfo) - Can return new damage amount
    ALL_CREATURE_EVENT_ON_MODIFY_HEAL_RECEIVED =
        12,  // (event, creature, target, heal, spellInfo) - Can return new heal amount
    ALL_CREATURE_EVENT_ON_DEAL_DAMAGE =
        13,  // (event, creature, target, damage, damagetype) - Can return new damage amount
