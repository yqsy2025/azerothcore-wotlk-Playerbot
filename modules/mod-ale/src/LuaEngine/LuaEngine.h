...
private:
    ...
    static int LoadScriptWithCache(lua_State* L, const std::string& filepath, bool isMoonScript,
                                   uint32* compiledCount = nullptr, uint32* cachedCount = nullptr);
    ...
    // The bodies of the templates are in HookHelpers.h, so if you want to use them you need to #include
    // "HookHelpers.h".
    template <typename K1, typename K2>
    int SetupStack(BindingMap<K1>* bindings1, BindingMap<K2>* bindings2, const K1& key1, const K2& key2,
    ...
    template <typename K1, typename K2>
    bool CallAllFunctionsBool(BindingMap<K1>* bindings1, BindingMap<K2>* bindings2, const K1& key1, const K2& key2,
...
public:
    ...
    int Register(lua_State* L, uint8 reg, uint32 entry, ObjectGuid guid, uint32 instanceId, uint32 event_id,
    ...
    void OnLuaStateOpen();
    bool OnAddonMessage(Player* sender, uint32 type, std::string& msg, Player* receiver, Guild* guild, Group* group,
                        Channel* channel);
    ...
    void OnCreatureModifyPeriodicDamageAurasTick(Creature* me, Unit* target, uint32& damage,
    ...
    void HandleGossipSelectOption(Player* pPlayer, uint32 menuId, uint32 sender, uint32 action,
    ...
    bool OnCanSendMail(Player* player, ObjectGuid receiverGuid, ObjectGuid mailbox, std::string& subject,
    ...
    void OnPlayerModifyPeriodicDamageAurasTick(Player* player, Unit* target, uint32& damage,
                                               SpellInfo const* spellInfo);
    ...
    void OnBankEvent(Guild* guild, uint8 eventType, uint8 tabId, uint32 playerGuid, uint32 itemOrMoney,
    ...
    void OnAllCreatureModifyPeriodicDamageAurasTick(Creature* me, Unit* target, uint32& damage,
                                                    SpellInfo const* spellInfo);
...
template <>
Unit* ALE::CHECKOBJ<Unit>(lua_State* L, int narg, bool error);
template <>
Object* ALE::CHECKOBJ<Object>(lua_State* L, int narg, bool error);
template <>
WorldObject* ALE::CHECKOBJ<WorldObject>(lua_State* L, int narg, bool error);
template <>
ALEObject* ALE::CHECKOBJ<ALEObject>(lua_State* L, int narg, bool error);
