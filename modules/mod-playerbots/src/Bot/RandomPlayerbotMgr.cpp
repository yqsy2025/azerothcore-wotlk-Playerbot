        ...
        if (uint8 type = BattlegroundMgr::BGArenaType(queueTypeId))
        {
            for (auto const& bracketIdPair : queueTypePair.second)
            {
                ...
                if (bgInfo.minLevel == 0)
                    continue;
                if (bgInfo.skirmishArenaInstanceCount == 0 && bgInfo.ratedArenaInstanceCount == 0)
                    continue;//当没有活动的竞技场禁用log显示
                LOG_INFO("playerbots",
...
bool RandomPlayerbotMgr::ProcessBot(Player* bot)
{
    ...
    if (group && group->GetLeader() && (IsRandomBot(group->GetLeader()) ||
         (botAI && botAI->GetMaster() && botAI->GetMaster()->isAFK())))  // 包含随机本也会退组
    {
        botAI->TellMaster("你不在我也撤了!");
...
void RandomPlayerbotMgr::RandomTeleport(Player* bot, std::vector<WorldLocation>& locs, bool hearth)
{
    ...
    if (bot->InBattleground() || bot->InArena() || bot->GetAreaId() == 2177)  // 2177: 古巴什大乱斗场
        return;
