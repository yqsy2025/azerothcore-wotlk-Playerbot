...
void PlayerbotAI::UpdateAIGroupMaster()
{
    ...
    if (group && !bot->InBattleground())
    {
        ...
        if (GetMaster() && IsRealPlayer(GetMaster()) && GetMaster()->IsAlive() &&
            (bot->IsInSameGroupWith(GetMaster()) || bot->IsInSameRaidWith(GetMaster())))
        {
            if (bot->GetMap() && GetMaster()->GetMap() && bot->GetMap() != GetMaster()->GetMap() &&
                !bot->GetMap()->IsDungeon() && !bot->IsBeingTeleported() && !GetMaster()->IsBeingTeleported())
            {
                if (bot->isDead())
                {
                ...
                bot->TeleportTo(GetMaster()->GetMapId(), GetMaster()->GetPositionX(), GetMaster()->GetPositionY(),
...
bool PlayerbotAI::SayToWorld(const std::string& msg)
{
    ...
    // no zone
    if (Channel* worldChannel = cMgr->GetChannel("世界频道", bot))
    {
...
Player* PlayerbotAI::FindNewMaster()
{
    ...
    bool leaderIsReal = groupLeader && !GET_PLAYERBOT_AI(groupLeader);

    // 2.1 先看团长是否在同小队且符合条件（避免一次完整遍历）
    if (leaderIsReal && group->SameSubGroup(bot, groupLeader))
    {
        // 如果团长在同小队且有助理标记，直接返回
        if (group->IsAssistant(groupLeader->GetGUID()))
            return groupLeader;
        // 否则记录为备选
    }

    // 2.2 遍历同小队查找
    Player* firstRealPlayer = nullptr;
    Player* firstAssistant = nullptr;
    ...
    for (GroupReference* gref = group->GetFirstMember(); gref; gref = gref->next())
    {
        ...
        // 记录第一个真实玩家
        if (!firstRealPlayer)
            firstRealPlayer = member;

        // 检查助理标记
        if (group->IsAssistant(member->GetGUID()))
        {
            firstAssistant = member;
            break;  // 找到助理就可以退出循环了
        }
    }
