...
void AuctionHouseBot::AddNewAuctions(std::vector<ObjectGuid> const& botGuids, FactionSpecificAuctionHouseConfig *config)
{
    if (botGuids.empty())
    {
        LOG_ERROR("module",
            "AHSeller: Empty player list");
        return;
    }
    ...
    for (uint32 cnt = 1; cnt <= newItemsToListCount; cnt++)
    {
        ...
        while (batchCount < 500 && itemsGenerated < newItemsToListCount)
        {
            ...
            ObjectGuid ownerGuid =
                botGuids[urand(0, botGuids.size() - 1)];
...
void AuctionHouseBot::AddNewAuctionBuyerBotBid(std::vector<ObjectGuid> const& botGuids, FactionSpecificAuctionHouseConfig *config)
{
    if (botGuids.empty())
    {
        LOG_ERROR("module",
            "AHBuyer: Empty player list");
        return;
    }
    if (!BuyingBotEnabled)
    {
    ...
    for (int count = 1; count <= randBuyingBotBuyCandidatesPerBuyCycle; ++count)
    {
        ...
        ObjectGuid bidderGuid = botGuids[urand(0, botGuids.size() - 1)];
...
void AuctionHouseBot::Update()
{
    ...
    for (auto const& bot : AHCharacters)
    {
        botGuids.push_back(
            ObjectGuid::Create<HighGuid::Player>(bot.CharacterGUID)
        );
    }
    // List New Auctions
    if (sellReady)
    {
        ...
        {
            AddNewAuctions(botGuids, &AllianceConfig);
            AddNewAuctions(botGuids, &HordeConfig);
        }

        AddNewAuctions(botGuids, &NeutralConfig);
    }
    ...
    if (buyReady && BuyingBotBuyCandidatesPerBuyCycleMin > 0)
    {
        if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_AUCTION))
        {
            AddNewAuctionBuyerBotBid(botGuids, &AllianceConfig);
            AddNewAuctionBuyerBotBid(botGuids, &HordeConfig);
        }

        AddNewAuctionBuyerBotBid(botGuids, &NeutralConfig);
    }
}
