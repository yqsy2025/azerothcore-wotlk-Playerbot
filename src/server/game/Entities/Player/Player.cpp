...
void Player::ApplyItemEquipSpell(Item* item, bool apply, bool form_change)
{
    if (!item)
        return;
    // 关键修复：只有装备在身上的物品才允许 apply
    if (apply && !item->IsEquipped())
        return;
    ItemTemplate const* proto = item->GetTemplate();
    if (!proto)
        return;
...
bool Player::InBattlegroundQueue(bool ignoreArena) const
{
    for (uint8 i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i)
        if (_BgBattlegroundQueueID[i].bgQueueTypeId != BATTLEGROUND_QUEUE_NONE &&
            (!ignoreArena || (_BgBattlegroundQueueID[i].bgQueueTypeId != BATTLEGROUND_QUEUE_2v2 && _BgBattlegroundQueueID[i].bgQueueTypeId != BATTLEGROUND_QUEUE_1v1 && //1v1
...
void Player::RefundItem(Item* item)
{
    ...
    // ========== 新增：随机附魔检查 ==========
    // 检查物品是否有随机附魔
    if (item->IsEnchanted() || (item->GetItemRandomPropertyId() && item->GetItemRandomPropertyId() == 10000))
    {
        // 发送错误提示给玩家
        ChatHandler(GetSession()).PSendSysMessage("带有随机附魔的物品不可退还");
        return;
    }
    // ========== 新增结束 ==========
