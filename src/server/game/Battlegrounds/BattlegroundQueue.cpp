...
void BattlegroundQueue::InviteGroupToBG(GroupQueueInfo* ginfo, Battleground* bg, TeamId teamId)
{
    ...
    for (auto const& itr : ginfo->Players)
    {
        ...
        {
            // Player is not in this queue anymore, skip修复crash触发断言
            LOG_DEBUG("bg.battleground", "Player {} ({}) is not in queue {} anymore, skipping invitation.",
                player->GetName(), player->GetGUID().ToString(), bgQueueTypeId);
            continue;
        }
