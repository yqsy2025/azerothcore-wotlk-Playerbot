...
bool Loot::FillLoot(uint32 lootId, LootStore const& store, Player* lootOwner, bool personal, bool noEmptyError, uint16 lootMode /*= LOOT_MODE_DEFAULT*/, WorldObject* lootSource /*= nullptr*/)
{
    ...
    /* //战场玩家死亡后掉落装备,类似传奇
    if (lootOwner->IsCorpse() && lootOwner->HasFlag(CORPSE_FIELD_DYNAMIC_FLAGS, CORPSE_DYNFLAG_LOOTABLE) &&
        (lootOwner->InBattleground() || lootOwner->InArena()))
    {
        // 获取玩家的已装备物品,战场玩家死亡后掉落装备
        ...
        {
            if (Item* item = lootOwner->GetItemByPos(INVENTORY_SLOT_BAG_0, slot))
            {
                equippedItems.push_back(item);
                totalItemLevel += item->GetTemplate()->ItemLevel;  // 获取装备的装等
                ++itemCount;
            }
        }
        if (equippedItems.empty() || itemCount == 0 || itemCount < 3 || totalItemLevel < 140)
            // 如果没有已装备物品，直接返回
            return false;
        // 计算玩家的平均装备装等
        float avgItemLevel = itemCount > 0 ? (float)totalItemLevel / itemCount : 0;

        // 根据装备装等调整掉率
        float dropChance = 0.02f;  // 默认掉率为 2%
        if (avgItemLevel >= 225) dropChance = 0.01f;
        else if (avgItemLevel > 150 && avgItemLevel < 225) dropChance = 0.1f;
        else if (avgItemLevel <= 150) dropChance = 0.9f;

        // 随机掉落玩家装备的任一物品
        if (!equippedItems.empty())
        {
            // 随机选择一件装备
            Item* randomEquippedItem = equippedItems[urand(0, equippedItems.size() - 1)];

            if (urand(1, 100) <= dropChance * 100) // 比如: dropChance 为0.02时有2%的概率通过
            {
                // 创建 LootStoreItem 来掉落这件装备
                LootStoreItem lootItem(
                    randomEquippedItem->GetEntry(), // 物品ID
                    0, // 参考ID（不引用）
                    dropChance * 100, // 掉落几率，100表示一定掉落
                    false, // 不需要任务
                    lootMode, // 掉落模式
                    0, // 掉落分组ID
                    1, // 最小掉落数量
                ...
                AddItem(lootItem);  // 将物品添加到战利品中
                lootOwner->DestroyItemCount(randomEquippedItem->GetEntry(), 1, true, 1);  // 从玩家身上移除该装备,可能crash宕机
            }
        }
    }
    */
