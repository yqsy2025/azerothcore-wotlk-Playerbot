...
bool WorldSocket::ReadHeaderHandler()
{
    ...
    if (!header->IsValidSize() || !header->IsValidOpcode())
    {
        LOG_ERROR("network", "WorldSocket::ReadHeaderHandler(): client {} sent malformed packet (size: {}, cmd: {})",
            ...
            std::string remoteIp = GetRemoteIpAddress().to_string();
            // 排除本地回环地址
            if (remoteIp == "127.0.0.1" || remoteIp == "::1" || remoteIp == "localhost")
            {
                LOG_DEBUG("network", "本地地址 {} 发送的非法数据包被忽略", remoteIp);
                return false;  // 只返回false，不记录违规
            }
            // 单线程环境，不需要锁
            static std::unordered_map<std::string, uint32> ipViolations;
            uint32 count = ++ipViolations[remoteIp];
            static time_t lastCleanupTime = 0;
            // 定期清理（每2小时）
            time_t now = time(nullptr);
            // 先检查是否需要清理（每2小时）
            if (now - lastCleanupTime > 7200) // 2小时 = 7200秒
            {
                ipViolations.clear();  // 清空所有记录
                lastCleanupTime = now;
                LOG_DEBUG("network", "清理违规记录表");
            }
            if (count >= 3)
            {
                if (sBan)
                {
                    sBan->BanIP(remoteIp, "0", "恶意数据包攻击", "系统");
                    LOG_INFO("network", "已封禁IP: {} (3次数据包违规)", remoteIp);
                }
                ipViolations.erase(remoteIp);
            }
        return false;
    }
